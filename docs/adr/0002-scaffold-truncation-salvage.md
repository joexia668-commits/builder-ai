...
这是独立问题，但会让 messages / versions 持久化偶发失败，给整个 debug 过程增加噪音（比如无法从 DB 查到某次失败的 architect message）。

根因：`lib/prisma.ts` 的 pg Pool 配置 (`max: 3, idleTimeoutMillis: 30000`) 不适合 Vercel serverless + Supabase pgbouncer 组合。Vercel 的 Lambda 实例会被冻结-解冻，解冻后池里的 TCP 连接已经被 pgbouncer 服务端丢弃，但 pool 还以为是 good connection，hand out 给下一次查询就 hang 到 15s connectionTimeout 报错。

修复：

```ts
max: 2,                    // 单实例低并发，配合 Supabase 免费档
idleTimeoutMillis: 5000,   // 快速释放，不让连接在 pool 里过期到 stale
connectionTimeoutMillis: 10000,
allowExitOnIdle: true,     // 请求结束后放行 event loop
```

并添加 `pool.on("error")` handler，防止一个坏 socket 的异步错误 crash 整个 serverless 实例。
