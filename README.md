...
BuilderAI is an AI-powered code generation platform. Describe your idea in natural language, and the system automatically handles requirements, architecture, and code generation through a modular pipeline (PM → Architect/Decomposer → Engineer). The generated app runs directly in a browser WebContainer with Vite HMR.
...
- **WebContainer 预览** — 多文件 React 应用在浏览器内真实 Node.js WASM 环境中运行，Vite HMR 增量热更新，支持完整 npm 生态（含 Phaser.js 等游戏引擎）→ [详情](docs/features/sandpack-preview.md)
...
```
用户输入 → classifyIntent() → classifyScene()
                │
    ┌───────────┴───────────────────────────┐
    ...
    │                                       │  PM Agent
    │                                       │    │
    │                                       │    ├── simple (≤3 modules, ≤5 features)
    │                                       │    │   Architect → layered Engineer (~60s)
    │                                       │    │
    │                                       │    └── complex (>3 modules OR >5 features)
    │                                       │        Decomposer → Skeleton
    │                                       │        → [Module Architect + Engineer] × N (~120s+)
    └───────────────────────────────────────┘
              WebContainer Preview (Vite HMR, incremental)
```

Engineer 按文件依赖关系**分层串行、层内并行**生成；内置三级容错：局部解析保留 → 仅失败文件重试 → 熔断。复杂路径由 PipelineController 状态机（IDLE → CLASSIFYING → DECOMPOSING → SKELETON → MODULE_FILLING → POST_PROCESSING → COMPLETE）统一编排。
...
| UI | shadcn/ui |
| Code Preview | WebContainer (@webcontainer/api, real Node.js WASM + Vite HMR), Monaco Editor |
