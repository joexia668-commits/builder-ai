...
- **Import/Export 一致性校验** — 生成后自动检测 named/default import 与 export 的不匹配，≤3 文件时发起定向修复请求
- **禁止包检测** — 扫描生成代码中的外部包引用，发现不可用包（如 react-router-dom）时自动重试修复
- **两阶段 Bug Fix** — bug_fix/style_change 先用轻量 triage 识别受影响文件，再只传这些文件给 Engineer，避免 DeepSeek 重写全部文件导致超时
...
- **迭代上下文记忆** — 最近 5 轮生成的需求摘要与架构决策持久化到数据库，PM 接收多轮历史写增量 PRD，Architect 在上一轮架构基础上增量修改；页面刷新后上下文不丢失
...
│   ├── agent-context.ts              # 各路径上下文拼装 + 迭代历史注入（PM/Arch）
...
| Import/Export 一致性校验 | 生成后交叉检查所有文件的 named/default import 与 export，≤3 文件时自动修复 |
| 禁止包引用检测 | 扫描外部 import 与允许列表（react/react-dom/lucide-react）对比，违规文件自动重试 |
| 两阶段 Bug Fix Triage | bug_fix 路径先用轻量 LLM 调用识别受影响文件（仅传路径不传代码），再只传这些文件做修复，防止 DeepSeek 重写全部文件超时 |
| Stream 超时精确识别 | Provider 层用 `abortController.signal.aborted` 区分内部超时与用户取消，前端显示正确错误类型 |
| 截断时部分救回 | `extractAnyMultiFileCode` 最后一个文件截断时，保留已完整的前 N-1 个文件 |
...
| 迭代上下文 FIFO-5 持久化 | `IterationContext { rounds[] }` 存在 `Project.iterationContext Json?`，FIFO 保留最近 5 轮；PM 收到多轮摘要写增量 PRD，Architect 收到上一轮架构决策做增量修改；`extractArchDecisions()` 从 ScaffoldData 确定性提取，不依赖 LLM 额外调用 |
